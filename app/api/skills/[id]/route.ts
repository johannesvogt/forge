...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await getSkillWithFiles(prisma as any, skill.name);
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const skill = await updateSkill(prisma as any, id, {
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  const existing = await getSkillById(prisma as any, id);
  if (!existing) return NextResponse.json({ error: 'Skill not found' }, { status: 404 });

  await deleteSkill(prisma as any, id);
